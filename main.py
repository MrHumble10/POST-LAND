...
# ___________________________morse sound_____________________________________>
...
def morse():
    logged_in = current_user.is_authenticated
    if not logged_in:
        flash("To enjoy the entertainments please log in.")
        return redirect(url_for("get_all_posts"))
    if request.method == 'POST':
        if request.form['p'] == 'from_morse':
            p = from_morse(request.form['message'])
        elif request.form['p'] == 'to_morse':
            p = to_morse(request.form['message'])
        elif request.form['p'] == 'morse_sound':
            p = return_sound(convert_dict, request.form['message'])
